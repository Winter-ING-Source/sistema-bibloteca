...
def extraer_datos_usuario(usuario):
    ...
    """
    cursor.execute(query_gustos, (usuario,))
    gustos = cursor.fetchall()
    
    # --- NUEVA CONSULTA: Extraer los títulos que ya leyó ---
    query_leidos = """
        SELECT l.titulo 
        FROM historial_lectura h
        JOIN usuarios u ON h.id_usuario = u.id_usuario
        ...
        WHERE u.nombre = %s;
    """
    cursor.execute(query_leidos, (usuario,))
    leidos = cursor.fetchall()
    
    cursor.close()
    conexion.close()
    return libros_crudos, gustos, leidos

def generar_lisp(libros, usuario, gustos, leidos):
    with open("datos_intermedios.lisp", "w", encoding="utf-8") as f:
        ...
        # --- NUEVO BUCLE: Escribir en LISP lo que ya leyó ---
        if leidos:
            for titulo in leidos:
                f.write(f'  (ya_leyo "{usuario}" "{titulo[0]}")\n')
        f.write(")\n")

def ejecutar_pipeline(usuario):
    try:
        # Ahora recibimos también la lista de 'leidos'
        libros, gustos, leidos = extraer_datos_usuario(usuario)
        if not gustos:
            return None, "El usuario no existe o no tiene historial."
            
        # Le pasamos la lista de 'leidos' al generador
        generar_lisp(libros, usuario, gustos, leidos)
